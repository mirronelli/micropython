...
void IRAM_ATTR esp_neopixel_write(uint8_t pin, uint8_t *pixels, uint32_t numBytes, uint8_t timing) {
    uint8_t *end;
    uint32_t time0, time1, period, c, startTime, pinMask;
    ...
    for (uint8_t* pixelByte = pixels; pixelByte < end; pixelByte++)         // iterate pixels bytes
    {
        uint8_t pixelValue = *pixelByte
        for (uint8_t mask = 0x80; mask != 0; mask >>= 1)                    // iterate bits in mask from b1000'0000  to b0000'0001
        {
            uint32_t bitDuration = (pixelValue & mask) > 0 ? time1 : time0; // duration of the bit signal
            
            GPIO_REG_WRITE(GPIO_OUT_W1TS_REG, pinMask);                     // Set high 
            startTime = mp_hal_ticks_cpu();                                 // Save start time
            
            while (mp_hal_ticks_cpu() - startTime < bitDuration);           // Wait for the bit high duration
            GPIO_REG_WRITE(GPIO_OUT_W1TC_REG, pinMask);                     // Set low

            while (mp_hal_ticks_cpu() - startTime < period);                // Wait for end of bit period
        }
    }
